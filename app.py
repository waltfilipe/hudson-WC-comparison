...
XT_MODEL_HEURISTIC_V3_1 = "heuristic_v3_1"
...
XT_MODEL_LABELS = {
    ...
    XT_MODEL_HEURISTIC_V3_1: "Heurístico v3.1 (zonas corrigidas)",
...
XT_MODEL_DESCRIPTIONS = {
    ...
    XT_MODEL_HEURISTIC_V3_1: "v3 + transição suave entre terços, finalização xG, monotonicidade, ΔxT coerente.",
...
XT_V3_1_FINE_NX = 96
XT_V3_1_FINE_NY = 64
XT_V3_1_DISPLAY_SUB = 24
XT_V3_1_ZONE_BLEND_WIDTH = 14.0
XT_V4_FINE_NX = 96
XT_V4_FINE_NY = 64
...
def classify_xt_progressive_for_model(
    ...
    if xt_model == XT_MODEL_HEURISTIC_V3_1:
        return classify_xt_progressive_v5(
            xt_start,
            delta_xt if delta_xt is not None else xt_end - xt_start,
            x_end,
...
def hypothetical_delta_xt(row, xt_model: str = XT_MODEL_HEURISTIC) -> float:
    ...
    if xt_model == XT_MODEL_HEURISTIC_V3_1:
        return _adjust_heuristic_v3_1_pass_delta(won_row)
...
def _map_zonal_threat_v3_1_smooth(x: np.ndarray) -> np.ndarray:
    """Blend zone scales with soft weights — lighter than v5 (14 m vs 26 m)."""
    blend = XT_V3_1_ZONE_BLEND_WIDTH
...
def _build_heuristic_v3_1_threat_surface(Xc: np.ndarray, Yc: np.ndarray) -> np.ndarray:
    zonal = _map_zonal_threat_v3_1_smooth(Xc)
...
@st.cache_data(show_spinner=False)
def compute_heuristic_v3_1_fine_grid(nx: int = XT_V3_1_FINE_NX, ny: int = XT_V3_1_FINE_NY) -> np.ndarray:
    xe = np.linspace(0.0, FIELD_X, nx)
    ye = np.linspace(0.0, FIELD_Y, ny)
    Xc, Yc = np.meshgrid(xe, ye)
    return _build_heuristic_v3_1_threat_surface(Xc, Yc)


@st.cache_data(show_spinner=False)
def compute_heuristic_v3_1_xt_grid(NX: int = NX_XT, NY: int = NY_XT, sub: int = XT_V3_1_DISPLAY_SUB) -> np.ndarray:
    ncols_hr = NX * sub
    nrows_hr = NY * sub
    xe = np.linspace(0, FIELD_X, ncols_hr + 1)
    ye = np.linspace(0, FIELD_Y, nrows_hr + 1)
    ...
    yc_arr = (ye[:-1] + ye[1:]) / 2
    Xc, Yc = np.meshgrid(xc, yc_arr)
    threat = _build_heuristic_v3_1_threat_surface(Xc, Yc)
    grid = np.zeros((NY, NX))
    for iy in range(NY):
        for ix in range(NX):
            grid[iy, ix] = threat[iy * sub:(iy + 1) * sub, ix * sub:(ix + 1) * sub].mean()
...
def _v3_1_short_pass_multiplier(pass_distance: float) -> float:
...
def _v3_1_zone_max_pass_delta(x_start: float) -> float:
...
def _adjust_heuristic_v3_1_pass_delta(row) -> float:
    """v3.1: smooth short-pass discount, zone cap, same recycle rules as v3."""
    if not row.is_won:
        return 0.0
    raw = float(row.xt_end - row.xt_start)
    if raw >= 0:
        adjusted = raw * _v3_1_short_pass_multiplier(row.pass_distance)
        return min(adjusted, _v3_1_zone_max_pass_delta(row.x_start))
    lat_start = _lateral_frac(row.y_start)
    lat_end = _lateral_frac(row.y_end)
    if row.x_start < XT_V3_NEG_RECYCLE_X_MAX:
        adjusted = raw * (XT_V3_NEG_PENALTY_FACTOR if lat_end < lat_start else 1.0)
...
def apply_heuristic_v3_1_xt(df: pd.DataFrame) -> pd.DataFrame:
    fine = compute_heuristic_v3_1_fine_grid()
    out = df.copy()
    out["xt_start"] = out.apply(lambda r: xt_value_bilinear(r["x_start"], r["y_start"], fine), axis=1)
    out["xt_end"] = out.apply(lambda r: xt_value_bilinear(r["x_end"], r["y_end"], fine), axis=1)
    out["delta_xt"] = out.apply(_adjust_heuristic_v3_1_pass_delta, axis=1)
    return out
...
def get_xt_grid(xt_model: str) -> np.ndarray:
    ...
    if xt_model == XT_MODEL_HEURISTIC_V3_1:
        return compute_heuristic_v3_1_xt_grid()
    if xt_model == XT_MODEL_HEURISTIC_V4:
...
def apply_xt_model(df: pd.DataFrame, xt_model: str) -> pd.DataFrame:
    ...
    elif xt_model == XT_MODEL_HEURISTIC_V3_1:
        out = apply_heuristic_v3_1_xt(df)
